''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
'
'  User2Configuration.vbs  
'  Copyright FineBuild Team © 2008 - 2018.  Distributed under Ms-Pl License
'
'  Purpose:      Script to perform user processing requirements after SQL Server has been installed.
'                This script should be changed as needed to perform the processing required.
'                
'                This script can use ProcessId values 5YA to 5ZZ
'
'  Author:       Ed Vassie
'
'  Change History
...
Dim strDirBackup, strDirData, strDirDataFT, strDirProg, strDirLog, strDirTempData, strDirTempLog, strDirSQL, strDrive, strDrives, strVolProg, strVolSys, strVolUsed
Dim strEdition, strFilePerm, strHKLMSQL, strInstance, strInstAgent, strInstAS, strInstIS, strInstNode, strInstRS, strInstSQL
...
' Error handling and termination
  ...
  Select Case True
    ...
    Case strProcessId > "5ZZ"
      ' Nothing
    Case err.Number = 0 
      Call objShell.Popup("User Configuration complete", 2, "Instance Configuration" ,64)
...
' Perform initialisation procesing
  ...
  strDrives         = GetBuildfileValue("VolList")
...
Sub ProcessUserConfig()
  Call SetProcessId("5U", "User Configuration processing (User2Configuration.vbs)")
...
' Main body of user processing is inserted here
  ...
  Call SetUpdate("OFF")
  Call SetProcessId("5ZZ", " User Configuration processing" & strStatusComplete)
  Call ProcessEnd("")
...
Sub SetupDrive(strInstance, strUserDrives, strUserLabel)
  Call SetProcessId("5YA", "Setup User drives")
...
Sub SetupThisDrive(strDrive, strDriveLabel)
  ...
  If Instr(strVolUsed, strDrive) = 0 Then       
    Call FBLog("Setup " & strDrive & ": drive for " & strDriveLabel)
    strVolUsed      = strVolUsed & strDrive
